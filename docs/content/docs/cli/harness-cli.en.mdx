...
mcp_toolset_id: ""        # Runtime binding (not an env var); applied on next deploy
knowledgebase:
...
| `--mcp-toolset-id` | Bind an AgentKit MCP Toolset; applied as a Runtime resource binding on deploy. |
