...
mcp_toolset_id: ""        # -> MCP_TOOLSET_ID + Runtime 关联资源；下次 deploy 生效
knowledgebase:
...
| `--mcp-toolset-id` | 绑定 AgentKit MCP Toolset，部署时注入 `MCP_TOOLSET_ID` 并作为 Runtime 关联资源生效。 |
...
2. **可在调用时覆盖的参数** —— 由 `HarnessOverrides` 派生的每个 `--<flag>`（`--model-name`、`--tools`、`--skills`、`--system-prompt`、`--runtime`）及其说明。记忆与知识库**不可**覆盖。AgentKit 的 harness invoke API 还接受一次性的 `mcp_toolset_id`，用于请求级 MCP Toolset 覆写；该字段不写回 `harness.yaml`。
